...
int main(void)
{
  ...
  nrf24_open_tx_pipe(Address_Node1);
  nrf24_stop_listen();
  //nrf24_open_rx_pipe(2, Address_Node3);
  ce_high();

  
  // sr04.trig_port = GPIOB;
  // sr04.trig_pin = GPIO_PIN_1;
  ...
  while (1)
  {
    ...
    // nrf24_type_to_uint8_t(Distance, data_T, PLD_SIZE);

    snprintf((char *)data_T, PLD_SIZE, "N1 d:%u ", Difference);

    uint8_t tx_status = nrf24_transmit(data_T, PLD_SIZE);

    if (tx_status == 0)  // Transmission successful and valid distance measurement
    {
