-- =============================================================================
-- SMART AWD RICE MONITORING â€” DATABASE SCHEMA (v3 â€” Industry Grade)
-- PostgreSQL 16 + PostGIS + TimescaleDB
-- Docker image: timescale/timescaledb-ha:pg16-latest
-- =============================================================================
-- Schema layout:
--   mst   â†’ master / reference data (users, devices, fields, sub_blocks, rules)
--   trx   â†’ transactional / operational data (telemetry, states, recommendations)
--   sys   â†’ system internals (scheduler, jobs, auth tokens, configs)
--   logs  â†’ audit & observability (access, errors, activity, data changes)
--
-- v3 additions from v2:
--   [mst]  + sensor_calibrations   â€” IoT sensor calibration offsets
--   [mst]  + alert_configs         â€” configurable alert thresholds
...
--   [trx]  + sub_block_current_states â€” CQRS current-state table (auto-synced)
--   [trx]  + telemetry_alerts       â€” real-time threshold violation alerts
--   [trx]  + weather_warning_snapshots â€” BMKG peringatan dini (nowcast warnings)
--   [logs] + data_change_audit      â€” audit trail for critical master data changes
...
-- =============================================================================
-- SCHEMA: mst
-- Urutan: tabel tanpa FK lebih dulu (referensi â†’ master â†’ relasi)
-- =============================================================================
...
CREATE TABLE mst.rice_duration_buckets (
  ...
  label        TEXT        NOT NULL,              -- 'Early (70â€“80 HST)'
...
CREATE TABLE mst.fields (
  ...
  description            TEXT,
  geom                   TEXT,
  adm4_code              VARCHAR(20) NOT NULL,    -- kode kelurahan Kepmendagri 2022 â†’ BMKG lookup
...
COMMENT ON TABLE mst.fields IS
  'Unit evaluasi DSS = satu field â†’ satu adm4_code â†’ satu konteks cuaca BMKG.';
...
COMMENT ON TABLE mst.user_fields IS
  'Mapping user â†” field untuk RBAC. Akses ke field otomatis mencakup semua sub-block di dalamnya.';
...
CREATE TABLE mst.sub_blocks (
  ...
  polygon_geom   TEXT                    NOT NULL,   -- polygon dari GeoJSON import (as string)
  area_m2        NUMERIC(12, 2),                     -- manual input or calculated in app (PostGIS disabled)
  centroid       TEXT,                               -- manual input or calculated in app (PostGIS disabled)
...
CREATE INDEX idx_sub_blocks_geom     ON mst.sub_blocks (polygon_geom);
CREATE INDEX idx_sub_blocks_centroid ON mst.sub_blocks (centroid);
...
-- ---------------------------------------------------------------------------
-- mst.devices   â† MASTER DEVICE SENSOR AWD
-- ---------------------------------------------------------------------------
...
-- Hierarki lookup: sub_block_id (paling spesifik) â†’ field_id â†’ NULL (global default)
...
CREATE TABLE mst.alert_configs (
  ...
  alert_type      TEXT          NOT NULL
                                CHECK (alert_type IN (
                                  'water_critical_low',    -- water_level_cm di bawah threshold â†’ segera irigasi
...
CREATE TABLE mst.irrigation_rule_profiles (
  ...
  -- Threshold AWD
  awd_lower_threshold_cm NUMERIC(6, 2) NOT NULL,  -- water level di bawah ini â†’ perlu irigasi
  awd_upper_target_cm    NUMERIC(6, 2) NOT NULL,  -- target water level setelah irigasi
  drought_alert_cm       NUMERIC(6, 2),            -- level kritis â†’ prioritas sangat tinggi
  -- Perilaku
  ...
  rain_delay_mm          NUMERIC(6, 2) NOT NULL DEFAULT 10.0, -- jika prakiraan hujan > N mm â†’ tunda irigasi
...
CREATE INDEX idx_map_layers_geom     ON mst.map_layers (bounds_geom);

-- =============================================================================
-- SCHEMA: sys â€” System Internals + Auth Tokens
-- =============================================================================
...
-- =============================================================================
-- SCHEMA: trx â€” Transactional / Operational Data
-- =============================================================================
...
COMMENT ON TABLE trx.raw_events IS
  'Payload mentah per device per event. Source of truth â€” tidak dimodifikasi setelah INSERT.';
...
-- ---------------------------------------------------------------------------
-- trx.telemetry_records   â† DATA SENSOR TERNORMALISASI (TimescaleDB HYPERTABLE)
-- ---------------------------------------------------------------------------
CREATE TABLE trx.telemetry_records (
  ...
  raw_event_id          UUID,                            -- ref ke raw_events (bukan FK â€” limitasi hypertable)
  ...
  created_at            TIMESTAMPTZ   NOT NULL DEFAULT now(),
  PRIMARY KEY (id)                                       -- PK simplified for standard PostgreSQL
);
...
-- ---------------------------------------------------------------------------
-- trx.sub_block_states  (history â€” semua state yang pernah ada)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- trx.sub_block_current_states  â† CQRS CURRENT-STATE TABLE
-- ---------------------------------------------------------------------------
...
-- TIDAK untuk query historis â€” gunakan trx.sub_block_states untuk itu.
...
-- Trigger: setiap INSERT ke sub_block_states â†’ UPSERT current state (hanya jika lebih baru)
...
BEGIN
  ...
  -- Only update if newer â€” prevent out-of-order overwrite
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE TABLE trx.weather_forecast_snapshots (
  ...
  temperature_c        NUMERIC(5, 2),
  humidity_pct         NUMERIC(5, 2),
  precipitation_mm     NUMERIC(7, 2),        -- field 'tp' BMKG (undocumented â€” handle null gracefully)
...
-- ---------------------------------------------------------------------------
-- trx.weather_warning_snapshots   (peringatan dini / nowcast BMKG â€” terpisah)
-- ---------------------------------------------------------------------------
-- BMKG menyediakan dua endpoint berbeda:
--   /prakiraan â†’ forecast 3 hari (di trx.weather_forecast_snapshots)
--   /peringatan â†’ nowcast warning aktif (di tabel ini)
...
CREATE TABLE trx.management_events (
  ...
  flag_expires_at     TIMESTAMPTZ NOT NULL,           -- manual/trigger calculated (immutability fix for Lite)
...
CREATE OR REPLACE FUNCTION set_flag_expires_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.flag_expires_at = (NEW.event_date::TIMESTAMPTZ + (NEW.flag_active_hours || ' hours')::INTERVAL);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_management_events_expires_at
  BEFORE INSERT OR UPDATE OF event_date, flag_active_hours ON trx.management_events
  FOR EACH ROW EXECUTE FUNCTION set_flag_expires_at();
...
-- ---------------------------------------------------------------------------
-- trx.telemetry_alerts   â† REAL-TIME THRESHOLD ALERTS
-- ---------------------------------------------------------------------------
...
-- =============================================================================
-- SCHEMA: logs â€” Audit & Observability
-- =============================================================================
...
INSERT INTO mst.rice_duration_buckets (bucket_code, label, hst_min, hst_max, sort_order) VALUES
  ('early',        'Early (70â€“80 HST)',         70, 80,  1),
  ('medium_early', 'Medium Early (90â€“100 HST)', 90, 100, 2),
  ('medium',       'Medium (110â€“120 HST)',      110, 120, 3),
  ('late',         'Late (120â€“140 HST)',         120, 140, 4);
...
VALUES
  -- EARLY (70â€“80 HST): 4 fase DSS aktif
...
-- =============================================================================
-- RINGKASAN TABEL FINAL â€” v3 (41 tabel)
-- =============================================================================
...
-- mst    | 04 | user_fields                     | RBAC: user â†” field
...
-- trx    | 24 | telemetry_records               | DATA SENSOR â€” hypertable
...
-- =============================================================================
-- Triggers (7 total):
--   trg_device_assignments_sync        â†’ sync mst.devices.sub_block_id
--   trg_sub_block_states_sync_current  â†’ upsert trx.sub_block_current_states
--   trg_recommendation_feedback_sync   â†’ update has_feedback di recommendations
--   trg_map_layers_publish_history     â†’ auto-record orthomosaic_publish_history
